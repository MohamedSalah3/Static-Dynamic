...
{	
	...
	while (1)
	{
		if(xEventGroupGetBits( LCD_EventGroup ) == Button_0_Flag)
		{
			...
			LCD_goto_xy(1,0);
			LCD_send_string("button one     ");

			vTaskDelayUntil( &xLastWakeTime, LCD_DISPLAY_PERIODICITY );
		}
		else if(xEventGroupGetBits( LCD_EventGroup ) == Button_1_Flag)

		{
			LCD_goto_xy(0,0);
			LCD_send_string("LED ON         ");
			LCD_goto_xy(1,0);
			LCD_send_string("button TWO     ");
